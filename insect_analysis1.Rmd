...
test = abund.order %>% filter(Site == 2007) %>% group_by(Order) %>% summarise(sum(Abund)) %>% rename(Abund = 'sum(Abund)')

ggplot(data = abund.order %>% filter(Site == 2013), aes(Order)) + geom_bar()
ggplot(data = abund.order %>% filter(Site == 2015), aes(Order)) + geom_bar()
